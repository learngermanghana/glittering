        ...
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <form className="rounded-3xl border border-black/10 bg-white p-6 shadow-lg sm:p-8" onSubmit={handleSubmit}>
            ...
            <div className="mt-5 grid gap-5 sm:grid-cols-2">
              <label className="text-sm font-semibold text-neutral-700">Appointment location<select name="branch" value={formData.branch} onChange={handleInputChange} required className={inputClassName("branch")}><option value="">Select location</option>{BRANCH_OPTIONS.map((branch) => <option key={branch.value} value={branch.value}>{branch.label}</option>)}</select><span className="mt-2 block text-xs font-normal text-neutral-500">This location helps us show the services available for that branch.</span>{fieldErrors.branch && <span className="mt-1 block text-xs font-normal text-red-600">{fieldErrors.branch}</span>}</label>
            ...
            <p className="mt-4 text-xs text-neutral-500">After payment, you will see a confirmation/verification page. Our team will contact you if anything needs attention.</p>
