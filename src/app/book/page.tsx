...
export default function BookPage() {
  ...
  const validationErrors = useMemo(() => {
    ...
    return errors;
  }, [formData, selectedServicePrice]);

  const progress = useMemo(() => {
    const checks = [formData.name, formData.phone, formData.email, formData.branch, formData.serviceId, formData.date, formData.time, formData.cancellationAccepted];
  ...
  return (
    <Container>
      <section className="rounded-[32px] bg-[#ffe6ea] px-4 py-12 sm:px-8 sm:py-16">
        <SectionTitle title="Book and Pay Online" subtitle="Choose your branch and service, then continue to secure Sedifex Checkout to reserve your slot." />
